---
name: Seed Pencils - Set of 4 boxes
slug: seed-pencils-set-of-4-boxes
sku: HSP001
category: seed-pencils-and-pens
categoryName: Seed Pencils & Pens
price: 470
salePrice: 376
currency: INR
availability: InStock
stockQuantity: 97
trackInventory: true
leadTimeDays: 3
ribbon: Most popular
bulk: false
customisable: false
options: []
customTextFields: []
variants: []
images:
- src: /assets/products/seed-pencils-set-of-4-boxes/seed-pencils-set-of-4-boxes-1.jpg
  alt: Four boxes of Eco Happy newspaper seed pencils fanned out on a cream cloth,
    seed capsules visible at the pencil tips
  width: 1350
  height: 1800
  original: originals/assets/products/seed-pencils-set-of-4-boxes/seed-pencils-set-of-4-boxes-1.jpg
- src: /assets/products/seed-pencils-set-of-4-boxes/seed-pencils-set-of-4-boxes-2.jpg
  alt: Close-up of a hand-rolled newspaper seed pencil showing the spiral newsprint
    body and sharpened graphite tip
  width: 832
  height: 1280
  original: originals/assets/products/seed-pencils-set-of-4-boxes/seed-pencils-set-of-4-boxes-2.jpg
- src: /assets/products/seed-pencils-set-of-4-boxes/seed-pencils-set-of-4-boxes-3.jpg
  alt: One open box of seed pencils next to three closed boxes, showing the plastic-free
    packaging
  width: 1350
  height: 1800
  original: originals/assets/products/seed-pencils-set-of-4-boxes/seed-pencils-set-of-4-boxes-3.jpg
wixCollections:
- MOST POPULAR
brand: Eco Happy
seoTitle: Seed Pencils - Set of 4 boxes | Eco Happy
seoDescription: Set of 4 boxes of newspaper seed pencils, hand-rolled in Pune. Write
  with them, then plant the stub in soil — no plastic packaging, no waste.
answer: Seed pencils are pencils hand-rolled from waste newspaper with a seed capsule
  at one end. Write with the pencil as usual; once it's too short to hold, plant the
  stub in soil, keep it watered, and it can sprout in one to three weeks depending
  on the variety and season.
specs:
  material: 100% recycled newspaper body, graphite core, embedded seed capsule
  madeIn: Pune, India
  madeBy: Araatrika women-led production team
  packaging: Plastic-free box
howToUse:
- Write with the pencil like any regular pencil until it's too short to sharpen.
- Plant the stub about 1–2 cm deep in a small pot of soil or your garden.
- Water it every day and keep it somewhere it gets a few hours of sunlight.
- Sprouting time varies by seed variety and season — check the box for what's inside.
faq:
- q: Does the seed pencil actually grow?
  a: Yes, as long as the stub is planted in soil, kept watered and gets sunlight.
    Germination depends on the seed variety and season — some sprout faster in the
    monsoon than in peak summer.
- q: What seeds are inside the pencils?
  a: Varieties differ by batch — common ones include tomato, chilli, coriander, spinach
    and tulsi. The box tells you which variety is inside.
- q: Is the graphite core safe, and is the pencil safe for children to use?
  a: The pencil writes like a normal graphite pencil and is safe for everyday use
    by children under normal supervision, the same as any pencil.
- q: How long does the newspaper pencil last before it needs planting?
  a: It lasts as long as a normal pencil of similar length — you plant the stub once
    it becomes too short to hold comfortably.
- q: Can I buy these in bulk for a school or corporate gift?
  a: Yes — message us on WhatsApp for bulk pricing and lead times; we also offer customised
    seed pencils with your name or logo (see our customised seed pencils).
related:
- grow-me-kit
- seed-pens-set-of-10
- newspaper-pencil-box-set-of-2
retired: false
datePublished: '2026-09-15'
dateModified: '2026-09-16'
migration:
  oldUrl: https://www.happypencils.in/product-page/eco-happy-seed-pencils-4-boxes
  wixId: 3bc75676-a10d-d2c3-4118-46993c80d5e7
  wixSku: HSP001
  todo:
  - Jagruti to review and personalise the description below before launch
  - confirm exact pencil count per box
---

Our best-selling product, and the easiest way to explain what Eco Happy makes: a pencil you write with today and plant tomorrow.

Each pencil is hand-rolled from waste newspaper by our production team in Pune, wrapped tight around a graphite core, with a seed capsule pressed into the blunt end instead of an eraser. You use it exactly like an ordinary pencil — sharpen it, write with it, carry it around — until it gets too short to hold. That's when the second life starts: plant the stub in a pot or a patch of soil, water it, and give it some sun. Depending on the seed variety and the season, you'll start seeing a sprout within one to three weeks.

This set comes as 4 boxes, each in plastic-free packaging, so nothing about the product or the box needs to go to landfill. It's the same reason schools, offices and event planners come back to this set for return gifts and Diwali hampers — it's a genuinely useful object first, and a conversation starter second.

We roll every pencil by hand in our Parvati Paytha workshop, the same place we've worked from since Araatrika started in 2017. If you're ordering more than a few sets — for a classroom, a wedding favour table or a corporate gifting order — message us on WhatsApp and we'll work out bulk pricing, lead time, and whether you'd like your name or logo added.
